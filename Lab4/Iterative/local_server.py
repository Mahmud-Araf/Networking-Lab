...
def handle_query(data, addr, server):
    ...
    packed_data = encode_dns_query(query, "",0)

    # Send to Root Server
    server.sendto(packed_data, (IP, ROOT_PORT))

    response, server_addr = server.recvfrom(BUFFER_SIZE)
    id, flag, q, a, auth_rr, add_rr, question, answer = response.decode(FORMAT)

    print(f"Local Server received response: {response}")
    i = 0
    while i<3:
        if validate_ip(response):
            server.sendto(response.encode(FORMAT), addr)
            ...
            next_server_port = int(response)
            server.sendto(query.encode(FORMAT), (IP, next_server_port))
